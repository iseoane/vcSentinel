...
| C-20 | Durable runs settled and verified | Terminal state plus verification per run | Sentinel | The gate emitted 5 runs — `8c7e7102`, `b1d10a68`, `452462b8`, `5634bcbb`, `712e1cbc` — each `succeeded` with `runs verify` reporting 4 intact events | met |
| C-21 | Zero-dimension reviews explained, not waved through | The empty scope has a recorded cause | Coordinator | FU-10: the planner reads only symbols and paths, so a documentation candidate cannot exceed `NivelNone`. Disclosed in the T9.4a record itself | met |
