...
| F8 | 🔄 | — | T8.1–T8.4 implemented and merged at `8f9a90a`. Exit criteria 1 and 3 were already covered by tests; **criterion 2 was unreachable from the PR commands** (blob reuse gated on `opts.Store != nil`, never assigned in production) and was closed in this pass with `resolveBlobStore` wired into `pr review` and `pr create` plus `TestStackedBranchSurvivesBaseRebaseViaBlob`. Pending: the §4.3 phase gate. Deviations recorded in the ficha. |
| F9 | ⬜ | — | |
