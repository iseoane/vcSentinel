...
| F8 | ✅ | `a244833` | T8.1–T8.4 implemented and merged at `8f9a90a`. Exit criteria 1 and 3 were already covered by tests; **criterion 2 was unreachable from the PR commands** (blob reuse gated on `opts.Store != nil`, never assigned in production) and was closed in `cd5c6e9` by wiring `resolveBlobStore` into `pr review` and `pr create`, with `TestStackedBranchSurvivesBaseRebaseViaBlob` over a real three-branch stack. `sentinel review cd5c6e9` returned `warn` on all five dimensions and its three accepted warnings were fixed in `7c401bf`. Phase gate §4.3: `go build`/`go vet`/`go test ./...` green, `./build.sh` OK, `sentinel gate --stage pre-push` **PASS** on `a244833`, stable guardian promoted. `sentinel pr review --base main` does not apply: all the work was committed straight to `main`, with no feature branch (same as F4). Deviations recorded in the ficha. Follow-up after closure (`ef63bf7`..`9a2880d`): the two residual limits were closed — the production wiring of both PR commands is now covered through `opcionesRamaPrReview`/`depsPrCreateReales`, and `internal/adaptersites` pins entries to source text (`Site.Anchor`) instead of absolute line numbers, removing the refresh churn that this phase alone triggered twice. Gate re-run **PASS** on `9a2880d`. |
| F9 | ✅ | `65c9edd` | T9.0–T9.5 cerradas y fusionadas en `main` (`1a355eb`..`65c9edd`); ramas y worktrees de fase retirados. Los cuatro criterios de salida se cierran con su alcance real, no con un aprobado plano: (1) **parcial** — `sentinel metrics` responde qué dimensión aporta hallazgos confirmados (78 confirmados, 1 refutado, cobertura `78/837`), pero no qué modelo genera ruido (FU-9, cobertura de identidad `0/325`) ni cuánto cuesta un hallazgo (FU-3, sin productor); ambos son clase A, no los arregla observar más; (2) **cumplido** — todo sale del common-dir local; (3) **cumplido una vez** — `review.timeout` de `300s`/`600s` a `900s` sobre 325 runs medidos, único eje que los datos sostenían; (4) **cumplido desde el guard de acuerdo, roto una vez antes** — la primera pasada de retención movió los contadores de éxito/fallo el 2026-09-04, de forma irreversible: las series que cruzan esa fecha no son comparables. T9.5 se desvía de su cláusula de aceptación original (conserva fichas y eventos, poda solo streams de ejecución) porque borrar fichas era insatisfacible junto con la medición byte-idéntica; desviación ratificada en la ficha. Tres bloqueos de revisión quedan registrados sin levantar (`c50d6d5`, `037c2db`, `7b375fd`): ningún escritor de producción emite `StatusRefuted`, así que hoy un humano no tiene forma de limpiarlos — es FU-6, fuera del alcance de esta fase. |
