...
|---|---|---|---|---|---|
| C-01 | `<named criterion>` | `<task line or behavior>` | `<writer/coordinator/vcSentinel>` | `<command; exit/status; result>` | `<met / blocked / follow-up>` |
...
| C-04 | Final review, run verification, and gate | `<vcSentinel record>` | vcSentinel | `<IDs, terminal states, verify, gate result>` | `<met / blocked>` |
...
REVIEW: Do not self-review, assign a semantic verdict, select review dimensions, or claim acceptance. vcSentinel review performs the review.
...
- Each confirmed correction follows the concrete vcSentinel flow: verify the premise, send it to the same writer or a replacement writer, create a new `fix(` commit, and re-review it. Do not stop while a vcSentinel block remains.
- Re-review every new `fix(` commit with vcSentinel. A blocked, unknown, unavailable, contradictory, or unverified result stays open until vcSentinel records a resolution; blockers never silently resolve.
...
- If a warning's premise is disproven, record why it is inert and do not revert a correct change. If only advisory findings remain and there is no production consumer, stop local correction and accept them only with a written reason recorded with the vcSentinel result.
...
- [ ] The final task candidate and every task commit have a vcSentinel review record; no review result is inferred from self-review.
...
Any unchecked item forbids completion. Do not stop while a vcSentinel block remains; self-review or prose cannot convert missing evidence into acceptance.
