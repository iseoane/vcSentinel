...
func firstLineIsUsage(output string) bool {
	...
	return strings.HasPrefix(first, "sentinel ")
}
...
// TestPrHelpDoesNotRunCleanup: 'sentinel pr --help' killed the legacy gh
...
func TestPrHelpDoesNotRunCleanup(t *testing.T) {
	for _, args := range [][]string{{"--help"}, {"-h"}, {"--title", "x", "--help"}} {
		...
		if !strings.Contains(out, "Usage") || !strings.Contains(out, "sentinel pr create") {
...
// TestHelpCommandMatchesHelpFlag: 'sentinel help <topic>' resolves through
...
func TestHelpCommandMatchesHelpFlag(t *testing.T) {
	for _, key := range expectedHelpKeys {
		...
		if !writeCommandHelp(&viaHelp, key) {
			t.Fatalf("%s: 'sentinel help %s' resolved no text", key, key)
		}
		subcommand, rest := invocationOfKey(key)
