...
// writeFakeSentinel writes a sh-script sentinel executable that mimics the
...
// repository common dir, pointing at the given sentinel executable — exactly
...
// when the sentinel binary rejects the staged candidate (exit 1) and ALLOW it
...
func TestPreCommitHookEnforcesStagedVolumeThroughRealGit(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		t.Skip("the fake sentinel fixture is a sh script; Windows coverage is compile-time here")
	}
	...
	root := prepareInitRepository(t)
	fakeSentinel := filepath.Join(root, "fakebin", "sentinel")
	hooksPath := installHookFor(t, root, fakeSentinel)

	// Over budget: fake sentinel exits 1, git commit must be blocked.
	writeFakeSentinel(t, fakeSentinel, true)
	...
	if !strings.Contains(string(output), "STAGED VOLUME REJECTED BY FAKE SENTINEL") {
	...
	// Within budget: fake sentinel exits 0, the same pipeline commits.
	writeFakeSentinel(t, fakeSentinel, false)
