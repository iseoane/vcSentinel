...
// all. Until T0.5 the dispatcher ignored os.Args[2:] and `sentinel check
...
func validateArguments(subcommand string, extras []string) string {
	...
	if subcommand == "slice" {
		...
		return fmt.Sprintf(
			"❌ 'sentinel slice' does not accept the argument %q. Use 'slice plan' or 'slice apply', or run 'sentinel help'.",
			extras[0])
	}
	...
	return fmt.Sprintf(
		"❌ 'sentinel %s' accepts no arguments and received: %s. Run 'sentinel help' to see the correct usage.",
