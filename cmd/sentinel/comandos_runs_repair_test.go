...
func TestRunsRecoverRepairRefusesNonUnprojectedClasses(t *testing.T) {
	...
	corruptBacking := store.NuevoStore(corruptRoot)
	...
	awaitingBacking := store.NuevoStore(awaitingRoot)
	...
	orphanedBacking := store.NuevoStore(orphanedRoot)
	...
	tests := []struct {
		...
		},
		{
			...
			backing: store.NuevoStore(t.TempDir()), runID: "missing-run-id",
