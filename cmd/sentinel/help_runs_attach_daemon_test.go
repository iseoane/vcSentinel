...
// text (start|status|stop), never the generic 'sentinel runs' header.
func TestHandleHelpServesDedicatedDaemon(t *testing.T) {
	...
	for _, args := range scenarios {
		...
		if strings.Contains(out, "sentinel runs <subcommand>") || out == runsUsage {
...
func TestDaemonSubcommandsRejectExtraFlags(t *testing.T) {
	...
	if !strings.Contains(out.String(), "❌ Unknown flag for 'sentinel runs daemon status': --json") {
