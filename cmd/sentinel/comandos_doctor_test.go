...
func TestValidarArgumentosDoctor(t *testing.T) {
	if msg := validarArgumentos("doctor", nil); msg != "" {
		t.Errorf("valid doctor args rejected: %s", msg)
	}
	if msg := validarArgumentos("doctor", []string{"--check-updates"}); msg != "" {
		t.Errorf("valid doctor flag rejected: %s", msg)
	}
	if msg := validarArgumentos("doctor", []string{"--json"}); msg == "" {
...
func TestEjecutarDoctorIsAdvisory(t *testing.T) {
	worktree := t.TempDir()
	out := &strings.Builder{}
	if code := ejecutarDoctorCon(out, worktree, "0.2.0", false, stubDoctorEnv(t)); code != 0 {
