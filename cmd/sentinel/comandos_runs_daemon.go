...
// executeRunsDaemon routes `sentinel runs daemon <subcommand>`. The three
...
func executeRunsDaemon(out io.Writer, worktree string, args []string) int {
	...
	if len(flags) > 0 {
		fmt.Fprintf(out, "❌ Unknown flag for 'sentinel runs daemon %s': %s\n", subcommand, flags[0])
		return runExitUsage
	}
	switch subcommand {
	case "start":
	...
	default:
		fmt.Fprintf(out, "❌ Unknown subcommand for 'sentinel runs daemon': %q\n", subcommand)
		return runExitUsage
	}
}
