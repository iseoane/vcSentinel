...
func (a *daemonProbeAgent) GetCommitMessage([]string, string, int) (string, error) {
	return "", nil
}

func (a *daemonProbeAgent) RunPrompt(prompt string) (string, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func startRunsTestDaemon(t *testing.T, worktree string) *daemonSideAdapter {
	...
	srv := daemon.NewServer(execution.NewController(store.NewStore(commonDir), adapter), daemon.FingerprintRepository(commonDir))
...
func TestRunsStartPrefersAHealthyDaemonEndpoint(t *testing.T) {
	...
	durable, err := store.NewStore(commonDir).ReadExecutionRequest(runID)
	if err != nil {
		t.Fatal(err)
	}
