...
func pruneRepoStore(t *testing.T, worktree string) *store.Store {
	...
	return store.NewStore(commonDir)
}
...
// ledger record survives pruning with its stable reason.
func TestRunsPruneKeepsLedgerReferencedStreams(t *testing.T) {
	...
	err = ledger.SaveRevision("fixtursha00000000000000000000000000000000", "fixture", "bucket", "model", review.Revision{
		At:     time.Now(),
		Result: "warn",
...
// the real collector over a real seeded ledger record (ticket 13 acceptance
// criterion 3): a dimension result carries its producer invocation while one
// of the revision's aggregated v2 findings carries the DIFFERENT admitted
// refuter invocation stamped at finalization. Aggregation may drop refuted
// findings from consumers that re-derive them, so the collector must scan
// the persisted AggregatedFindings directly — both identities must land in
// the reference set.
func TestCollectProvenanceReferencesIncludesRefutedFindingInvocations(t *testing.T) {
	...
	ledger := review.NewLedger(commonDir)
	err = ledger.SaveRevision("fixtursha00000000000000000000000000000001", "fixture", "bucket", "model", review.Revision{
		At:     time.Now(),
		Result: "warn",
		Dims: []review.DimensionResult{{
			Dim:          "logic",
...
func TestCollectProvenanceReferencesIncludesLinkedWorktreeLedgers(t *testing.T) {
	...
	linkedInvocation := "inv-producer-from-linked-worktree"
	ledger := review.NewLedger(linkedGitDir)
	if err := ledger.SaveRevision("fixtursha00000000000000000000000000000002", "fixture", "bucket", "model", review.Revision{
		At:     time.Now(),
		Result: "warn",
		Dims: []review.DimensionResult{{
			Dim:          "logic",
			Verdict:      "warn",
			InvocationID: linkedInvocation,
		}},
	}); err != nil {
		t.Fatal(err)
	}
	...
	if !references[linkedInvocation] {
		t.Fatalf("collectProvenanceReferences() = %v, missing %q cited by a record in the linked worktree ledger %q; a prune would destroy the stream it protects",
			references, linkedInvocation, linkedGitDir)
	}
}

// TestLedgerV1DirectoriesFailsClosed is the oracle for every way this
...
// wrote a record". A regular file where the ledger belongs fails an IsDir check
...
func TestLedgerV1DirectoriesFailsClosed(t *testing.T) {
	cases := []struct {
		name  string
		stage func(t *testing.T, commonDir string)
		want  string
	}{
		{
			name: "the worktrees path is not a directory",
			stage: func(t *testing.T, commonDir string) {
		...
		{
			name: "the ledger path is a regular file",
			stage: func(t *testing.T, commonDir string) {
		...
		{
			name: "the ledger path is a dangling symlink",
			stage: func(t *testing.T, commonDir string) {
				gitDir := filepath.Join(commonDir, "worktrees", "linked")
				if err := os.MkdirAll(gitDir, 0o755); err != nil {
					t.Fatal(err)
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			commonDir := t.TempDir()
			c.stage(t, commonDir)
			_, err := ledgerV1Directories(commonDir)
			if err == nil {
			...
			if !strings.Contains(err.Error(), c.want) {
				t.Errorf("error = %q, want it to mention %q so the operator can tell which path is wrong", err, c.want)
			}
		})
	}
}

// TestLedgerV1DirectoriesFindsRealLedgers is the positive half. Without
...
func TestLedgerV1DirectoriesFindsRealLedgers(t *testing.T) {
	t.Run("no linked worktrees", func(t *testing.T) {
		commonDir := t.TempDir()
		directories, err := ledgerV1Directories(commonDir)
		if err != nil {
			t.Fatalf("ledgerV1Directories() error = %v", err)
		}
		if !slices.Equal(directories, []string{commonDir}) {
			t.Errorf("ledgerV1Directories() = %v, want only the common directory", directories)
		}
	})
	...
	t.Run("one with a ledger and one without", func(t *testing.T) {
		...
		want := []string{commonDir, filepath.Join(commonDir, "worktrees", "with-records")}
		if !slices.Equal(directories, want) {
			t.Errorf("ledgerV1Directories() = %v, want %v", directories, want)
		}
	})
}
