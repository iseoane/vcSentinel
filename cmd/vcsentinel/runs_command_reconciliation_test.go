...
func TestRunsObservationSurfacesReconcileOwnerDeathDuringCancellation(t *testing.T) {
	...
	if code := inspectExecution(&out, host, backing, orphaned, "test-harness", false); code != runExitSuccess {
	...
	out.Reset()
	if code := inspectExecution(&out, host, backing, settled, "test-harness", false); code != runExitSuccess {
	...
	out.Reset()
	if code := inspectExecution(&out, host, backing, orphaned, "test-harness", true); code != runExitSuccess {
	...
	out.Reset()
	if code := inspectExecution(&out, host, backing, settled, "test-harness", true); code != runExitSuccess {
...
func TestRunsStatusListingSurfacesRunsNeedingAttention(t *testing.T) {
	...
	rendered := out.String()
	for _, want := range []string{"1 run(s) were left unsettled", "nothing is driving them", "vcsentinel runs recover"} {
...
func TestReportUnsettledRunsAlwaysNamesAFailedScan(t *testing.T) {
	...
	for _, want := range []string{"2 run(s) were left unsettled", "vcsentinel runs recover"} {
