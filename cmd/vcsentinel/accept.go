...
func parseAcceptArgs(args []string) (acceptOptions, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--reason":
			...
			opts.reason = args[i]
		default:
			return opts, fmt.Errorf("unknown flag %q (usage: vcsentinel accept --sha SHA --fingerprint FP --reason TEXT)", args[i])
		}
	}
...
// runAccept implements `vcsentinel accept`: parse, resolve, record.
