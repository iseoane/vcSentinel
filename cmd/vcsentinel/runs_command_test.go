...
func (a *fakeRunsAgent) GetCommitMessage([]string, string, int) (string, error) {
	return "", errors.New("fakeRunsAgent: commit messages are not part of these tests")
}

func (a *fakeRunsAgent) RunPrompt(string) (string, error) {
	if a.err != nil {
		return "", a.err
	}
	return a.output, nil
...
var _ agentadapter.PromptAdapter = (*fakeRunsAgent)(nil)
...
func (a *contextualTreeAgent) RunPromptWithContext(ctx context.Context, prompt string) (string, error) {
	a.contextualCalls++
	a.receivedCtx = ctx
...
// speaking only the legacy RunPrompt contract keep working unchanged.
...
func replaceRunsAgent(t *testing.T, agent agentadapter.PromptAdapter) {
	t.Helper()
	previous := newAgentForRuns
	newAgentForRuns = func(string) (agentadapter.PromptAdapter, error) { return agent, nil }
...
func TestExecuteRunsUsageErrorsExitOne(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantUso && !strings.Contains(output, "vcsentinel runs") {
...
func TestRunsStartAdmitsAndCompletesThroughConfiguredAgent(t *testing.T) {
	...
	metrics, err := store.NewStore(commonDir).ReadExecutionMetrics(decoded["run_id"].(string))
	if err != nil {
		t.Fatal(err)
	}
