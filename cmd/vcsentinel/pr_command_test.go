...
func TestPrVerb(t *testing.T) {
	...
	if exitCode != 1 ||
		...
		!strings.Contains(message, "vcsentinel pr create") ||
		!strings.Contains(message, "vcsentinel pr review") {
...
func TestParsePrReviewFlags(t *testing.T) {
	...
	} else if !strings.Contains(err.Error(), "vcsentinel review <sha>") {
...
func TestParsePrCreateFlags(t *testing.T) {
	...
	if _, err := parsePrCreateFlags([]string{"--audit-pending"}); err == nil || !strings.Contains(err.Error(), "vcsentinel review <sha>") {
...
// them, and point at 'vcsentinel review <sha>' — but it must never block, and
...
func TestRunPrReviewReportsUnauditedCommitsWithoutBlocking(t *testing.T) {
	...
	for _, want := range []string{"no review record", "abc1234a", "feat(x): x", "vcsentinel review abc1234abcd", "PR decision"} {
