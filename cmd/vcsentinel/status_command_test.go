...
// worktree (never to the real cwd of the vcsentinel repo): if the fix under
...
// TestStatusAcceptsBareInvocation reproduces the real bug: "vcsentinel status"
...
func TestStatusAcceptsBareInvocation(t *testing.T) {
	...
	if flagsNotApplicableToStatus(flags) {
		t.Error("vcsentinel status with no arguments should be accepted, not rejected")
	}
}
...
// vcSentinel runs inside its own pre-commit hook, which is exactly a context
...
// This drives the real `vcsentinel review` command twice against the same commit
