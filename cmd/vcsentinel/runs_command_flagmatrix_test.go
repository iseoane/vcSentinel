// Strict per-subcommand flag parsing tests for `vcsentinel runs` (ticket 13
	...
	for _, subcommand := range []string{"start", "status", "logs", "respond", "abort", "retry", "recover", "verify", "prune"} {
		...
		for _, flag := range runsAllFlags {
			...
			if !wantAccept {
				...
				} else if !strings.Contains(err.Error(), "not accepted by 'vcsentinel runs "+subcommand+"'") {
...
func TestExecuteRunsRejectsForeignSubcommandFlags(t *testing.T) {
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if !strings.Contains(output, "not accepted by 'vcsentinel runs") {
