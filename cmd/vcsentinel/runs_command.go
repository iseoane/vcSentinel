...
func executeRuns(out io.Writer, worktree string, args []string) int {
	...
	default:
		fmt.Fprintf(out, "❌ Unknown subcommand for 'vcsentinel runs': %q\n%s", subcommand, runsUsage)
		return runExitUsage
	}
}

// runsFlag identifies one declared flag of the `vcsentinel runs` CLI surface.
...
func parseRunOptions(subcommand string, args []string) (runOptions, error) {
	...
	if !ok {
		return options, fmt.Errorf("unknown subcommand for 'vcsentinel runs': %s", subcommand)
	}
	...
	for i := 0; i < len(args); i++ {
		...
		if !accepted[runsFlag(flag)] {
			return options, fmt.Errorf("flag %s is not accepted by 'vcsentinel runs %s'", flag, subcommand)
		}
		...
		default:
			err = fmt.Errorf("unknown flag for 'vcsentinel runs %s': %s", subcommand, flag)
		}
		if err != nil {
			return options, err
		}
...
func buildRunsStore(worktree string) (*store.Store, error) {
	...
	return store.NewStore(gitCommonDir), nil
}
...
func newPromptRunAdapter(agent agentadapter.PromptAdapter) (promptRunAdapter, error) {
	if agent == nil {
...
// does through RunPromptWithContext), cancellation propagates down to
// the owned child process tree; delegates speaking only the legacy
// RunPrompt contract behave exactly as before.
func (a promptRunAdapter) runPrompt(ctx context.Context, prompt string) (string, error) {
	if contextual, ok := a.delegate.(interface {
		...
		return contextual.RunPromptWithContext(ctx, prompt)
	}
	return a.delegate.RunPrompt(prompt)
}
...
// TreeProvider shape structurally, so `vcsentinel runs abort` escalates
