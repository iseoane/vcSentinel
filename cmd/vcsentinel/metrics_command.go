...
func parseMetricsArgs(args []string) (metricsFlags, error) {
	...
	for _, arg := range args {
		if arg != "--json" {
			return flags, fmt.Errorf("vcsentinel metrics accepts only --json, received %q", arg)
		}
		flags.jsonOut = true
	}
	return flags, nil
...
func renderMetrics(out io.Writer, report metrics.Report) error {
	...
	fmt.Fprintf(&text, "  duration (nanoseconds): %s\n  input tokens: %s\n  output tokens: %s\n  total tokens: %s\n  cached input tokens: %s\n  cache-write input tokens: %s\n  reasoning tokens: %s\n", formatMeasurement(e.Duration), formatMeasurement(e.InputTokens), formatMeasurement(e.OutputTokens), formatMeasurement(e.TotalTokens), formatMeasurement(e.CachedInputTokens), formatMeasurement(e.CacheWriteInputTokens), formatMeasurement(e.ReasoningTokens))
