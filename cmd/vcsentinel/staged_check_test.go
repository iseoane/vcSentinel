...
func TestHookInvokesStagedContractWithPortablePath(t *testing.T) {
	...
	executable := filepath.Join(dir, "vcsentinel binary")
	...
	if err != nil {
		t.Fatalf("hook did not invoke the vcsentinel executable: %v", err)
	}
	if got := string(args); got != "check\n--staged\n" {
		t.Fatalf("hook arguments = %q, want check --staged", got)
	}
	if !strings.Contains(generateHookScriptFor(filepath.FromSlash("C:/Program Files/vcsentinel")), "C:/Program Files/vcsentinel") {
