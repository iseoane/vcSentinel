...
// `vcsentinel runs` exit codes and terminal-state mapping. Every subcommand
...
const runsUsage = `vcsentinel runs <subcommand> [flags]
...
type promptRunAdapter struct {
	delegate agentadapter.PromptAdapter
}
...
var newAgentForRuns = func(worktree string) (agentadapter.PromptAdapter, error) {
	cfg, err := config.LoadStrictLocalConfig(worktree)
	if err != nil {
		return nil, err
	}
	profile := config.ResolveProfile(cfg, "", "")
	return agentadapter.NewAdapterWithProfile(cfg, profile)
}
...
// inspectorDurable is the only thing observeUntilSettled needs from the
// controller. It is an interface so the tolerance to a tail caught
// mid-append can be tested without standing up a real run:
// *execution.Controller satisfies it.
type inspectorDurable interface {
	Inspect(ctx context.Context, runID agentrun.Identity) (execution.Inspection, error)
}

// maxMidWriteTails caps how many consecutive observations of an incomplete
// tail are tolerated before reporting it. With runsObserveInterval at 50 ms
// that is ~2 s: plenty for a concurrent append to finish, and short enough
// not to hang the command on a genuinely truncated tail.
const maxMidWriteTails = 40
...
func observeUntilSettled(ctx context.Context, c inspectorDurable, runID agentrun.Identity) (store.RunProjection, error) {
	...
	for {
		...
		if err != nil {
			// Store readers are deliberately lock-free while the writer
			// does take the lock (see ReadAttemptOutcomes), so a reader
			// can see the last JSONL record mid-append. The store calls
			// that state "recoverable" because it resolves itself the
			// moment the write finishes: it is a "not yet", not a failure.
			// Treating it as an error turned a benign race into
			// REVIEW_INFRASTRUCTURE_ERROR.
			//
			// A genuinely truncated tail, left behind by a process that
			// died mid-write, never completes: that is why the tolerance
			// is bounded and the error eventually surfaces with its
			// identity intact.
			if errors.Is(err, store.ErrIncompleteEventTail) && tailsFollowed < maxMidWriteTails {
				tailsFollowed++
				select {
				case <-ctx.Done():
