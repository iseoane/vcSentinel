...
// `vcsentinel tui` (control-center slice 9): opens the Bubble Tea control
...
func spawnDetachedTuiDaemon() (tuiDaemonProcess, error) {
	...
	if err != nil {
		return tuiDaemonProcess{}, fmt.Errorf("could not resolve the vcsentinel binary: %w", err)
	}
...
// durable-runs plumbing the `vcsentinel runs` CLI uses, restricted to the
...
// executeTui dispatches `vcsentinel tui`. Registry-open failure exits
