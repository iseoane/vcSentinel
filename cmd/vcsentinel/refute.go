...
func parseRefuteArgs(args []string) (refuteOptions, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--line-end":
			...
			opts.lineEnd = n
		default:
			return opts, fmt.Errorf("unknown flag %q (usage: vcsentinel refute --sha SHA --fingerprint FP --reason TEXT --line-start N --line-end M)", args[i])
		}
	}
...
// runRefute implements `vcsentinel refute`: parse, resolve, record.
