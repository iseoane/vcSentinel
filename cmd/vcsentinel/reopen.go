...
func parseReopenArgs(args []string) (reopenOptions, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--line-end":
			...
			opts.lineEnd = n
		default:
			return opts, fmt.Errorf("unknown flag %q (usage: vcsentinel reopen --sha SHA --fingerprint FP --reason TEXT --line-start N --line-end M)", args[i])
		}
	}
...
// runReopen implements `vcsentinel reopen`: parse, resolve, record.
