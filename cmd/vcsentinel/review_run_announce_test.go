// Command-level tests for the `vcsentinel review` admitted-run announcement
...
// operator can immediately run `vcsentinel runs attach --run <id> --follow`.
// The announcement is wired ONLY at the vcsentinel-review command boundary and
...
func attachCommandFor(runID string) string {
	return "vcsentinel runs attach --run " + runID + " --follow"
}
...
// TestVCSentinelReviewWiringAnnouncesAdmittedRunsOnStderr drives the EXACT
...
func TestVCSentinelReviewWiringAnnouncesAdmittedRunsOnStderr(t *testing.T) {
