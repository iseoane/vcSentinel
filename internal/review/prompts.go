...
- If there is nothing to report, return {"dim": %q, "verdict": "ok"}.
	- Output ONLY one JSONL object between BEGIN_REVIEW and END_REVIEW. No markdown outside the delimiters. No commentary. Keys: dim, verdict, findings (dimension, file, line, severity, description, suggestion, evidence, confidence), questions (id, text, file), reason. Omit file on a question if you cannot attribute it to exactly one existing file — never invent or guess a path.`+seccionRespuestas+`
BEGIN_REVIEW
