...
func TestRegisterRefreshesNameAndIsIdempotent(t *testing.T) {
	...
	mustDo(t, os.Mkdir(registryDir, 0755))
	...
	mustDo(t, os.WriteFile(path, []byte(data), 0644))

	store := mustOrPanic(Open(path))
	if runtime.GOOS != "windows" {
		directoryInfo := mustOrPanic(os.Stat(registryDir))
		fileInfo := mustOrPanic(os.Stat(path))
		require(t, directoryInfo.Mode().Perm() == 0700 && fileInfo.Mode().Perm() == 0600, "Open() modes = directory %o, file %o; want 0700 and 0600", directoryInfo.Mode().Perm(), fileInfo.Mode().Perm())
	}
	...
	require(t, err == nil && !added && store.Len() == 1, "second Register() = (%v, %v), len = %d", added, err, store.Len())
	entry := store.List()[0]
...
func TestRegistryLifecycleSortedAndMissing(t *testing.T) {
	...
	require(t, store.Len() == 0, "Len() after Open() = %d, want 0", store.Len())
	t.Chdir(root)
	mustOrPanic(store.Register("b" + string(filepath.Separator)))
	mustOrPanic(store.Register(filepath.Join(root, "a")))
	entries := store.List()
	require(t, store.Len() == 2 && len(entries) == 2 && entries[0].Path == filepath.Join(root, "a") && entries[1].Path == filepath.Join(root, "b"), "List() = %+v, want sorted normalized paths", entries)
