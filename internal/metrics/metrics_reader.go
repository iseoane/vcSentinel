	...
	for _, sha := range shas {
		...
		for revisionIndex, revision := range ficha.Revisions {
			for _, finding := range revision.HallazgosEfectivos() {
				...
				if finding.Status == review.StatusReopened {
					input.Remediations = append(input.Remediations, RemediationObservation{
...
func remediationFromEvent(event ops.Evento) (RemediationObservation, bool) {
	...
	if logicalID == "" {
		logicalID = "event:" + event.At.UTC().Format(time.RFC3339Nano) + ":" + event.Cmd
	}
	return RemediationObservation{
		Target: stringField(fields, "fingerprint", "target"), LogicalID: logicalID,
		Dimension: stringField(fields, "dimension"), Success: success, At: event.At,
	}, true
}
