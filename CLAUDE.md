...
- Publicar assets de release: `go run ./tools/release` genera los binarios multiplataforma en `bin/<version>/`.
...
- `cmd/main.go` — entrypoint CLI: subcomandos `init`, `check`, `slice`
...
- `internal/git` — `CheckDiffLimits` (umbral de volumen), `ObtenerArchivosModificados` (numstat), `FragmentarYCommitear`
...
- `slice`: fragmenta por capas en orden fijo `config → backend → frontend → test`, lotes de ≤400 líneas, cada lote es un commit.
